"""
Opens a REAL draft PR on YOUR GitHub while faking Jira -- for validating
the git/GitHub wiring in pipeline.py end to end without needing a Jira
account. The Planner/Coder/Reviewer are real Claude calls; the branch,
commit, and push are real local git against repo_path; create_pull_request
is a real call to the GitHub MCP server. Only jira_get_issue/jira_add_comment
are faked, with a made-up ticket -- there's no Jira account here, so
nothing else could reasonably be faked.

Costs real Anthropic API calls and pushes a real branch + opens a real
draft PR on the repo you point it at. Use a throwaway test repo, not
something that matters.

Requires:
  ANTHROPIC_API_KEY, GITHUB_PAT (repo scope)
...
async def main() -> None:
    missing = [v for v in ("ANTHROPIC_API_KEY", "GITHUB_PAT") if not os.environ.get(v)]
    if missing:
